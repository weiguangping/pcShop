<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFiler" :key="index" :class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddId=item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click='modelShow(item.addressId)'>
      ...
      <modal :mdShow="modelConfirm" v-on:close="modelConfirm = false">
        <p slot="message">确定删除么？</p>
        <div slot="btnGroup">
...
<script>
  export default{
    ...
    methods: {
      delAddress () {
        this.$ajax.post('/api/users/delAddress', {
      ...
      setDefault (id) {
        this.$ajax.post('/api/users/setDeafault', {
          addressId: id
        }).then(response => {
          let res = response.data
          if (res.status === '0') {
      ...
      init () {
        this.$ajax.get('/api/users/addressList').then(response => {
          let res = response.data
          if (res.status === '0') {
            // console.log(res)
